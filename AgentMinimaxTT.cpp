...
int AgentMinimaxTT::playColumn(BoardState state) {
    if (isPlayer1) {
        int bestMove = -1;
...
int AgentMinimaxTT::minimax(BoardState state, int depth, int alpha, int beta, bool isCurrentPlayer1) {
    if (depth == 0 || state.checkWinningState()) {
        ...
        return transpositionTable[state.getKey(isCurrentPlayer1)];
    }
...
int AgentMinimaxTT::heuristicEvaluation(BoardState state, int depth) {
...
int AgentMinimaxTT::countWinningStates(uint64_t bitmap) {
    int winningStates = 0;
...
int AgentMinimaxTT::countBits(uint64_t bitmap) {
